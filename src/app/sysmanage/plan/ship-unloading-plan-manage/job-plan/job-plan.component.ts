...
@Component({
  ...
  styleUrls: ['./job-plan.component.css']
})
export class JobPlanComponent implements OnInit {
  ...
  boatTeam: Array<any> = [];
  x: any;
  loadingSequenceTmp: any; //调度计划明细 未选列表
   ...
   * @param data
   */
  leftupdateDataResult(data) {
    this.modelLeft.forEach((item, index) => {
      if (data && data.rowIndex == item.rowIndex) {
        data.expectEndTime = null;
        data.expectStartTime = null;
   ...
   */
  export() {
    let url: any = planUrl.boatWorkexport;
   ...
   */
  exportDetail() {
    let url: any = planUrl.itemWorkexport;
