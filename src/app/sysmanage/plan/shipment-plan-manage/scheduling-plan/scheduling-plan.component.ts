...
import {Bundle, ShipPlan, ShipPlanStatus} from '../ship-loading/ship-plan-status.enum';
...
export class SchedulingPlanComponent implements OnInit {
  ...
  fileType: string = 'application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'; //文件上传类型
  ...
  getPackList(param: any): void {
    ...
    this.http.post(planUrl.getPackList, param).then(
      res => {
        this.spinning = false;
        if (res.success) {
          this.tmpButtonId = '';
  ...
  btnUpdate(): void {
    ...
    const status = this.selectedData.some(
      res => !ShipPlan.ShipPlanReleaseableStatus(res.boatStatus)
    );
    if (status) {
  ...
  deleteData(): Promise<any> {
    const url = planUrl.boatScheduleDeleteList;
  ...
  issued(): void {
    ...
    const status = this.selectedData.some(
      res => res.status !== 'CBDD10' && res.status !== 'CBDD70'
    );
    if (status) {
  ...
  addDetailData(): void {
    ...
    this.detailDataModalList = [{EDIT: true, inputDisabled: this.inputDisabled, EDITSTATUS: true}, ...this.detailDataModalList];
  }
  ...
  addPackData(): void {
    ...
    if (this.selectedData.length == 0) {
      ...
      this.packDataSet = [{EDIT: true, inputDisabled: this.inputDisabled, EDITSTATUS: true}, ...this.packDataSet];
    }
  }
