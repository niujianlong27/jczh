...
@Component({
  selector: 'app-stock-set',
  templateUrl: './stock-set.component.html',
  styleUrls: ['./stock-set.component.css']
})
export class StockSetComponent implements OnInit {
  ...
  handleOk(){
    ...
    if( this.status === 'Add'){
      ...
      param.stockName = param.stockName.trim();
      this.http.post(stockUrl.selectStockSetName,{}).then((x:any)=>{
        if(x.success){
          let temp = x.data.data || [];
          if( !JSON.stringify(temp).includes(param.stockName) ){
            this.http.post(stockUrl.insertStockSet,param).then((res:any)=>{
              if(res.success){
                this.modalFormVisible = false;
                this.getList1({ page: 1, length: this.pageSize1 });
                this.nm.info({
      ...
      this.http.post(stockUrl.updateStockSet,param).then((res:any)=>{
        if(res.success){
          this.modalFormVisible = false;
          this.getList1({ page: 1, length: this.pageSize1 });
          this.nm.info({
  ...
  modalConfirmResult(data:any){
   ...
   if(data.type === 'ok'){
     let params = {stockSetModels:[]};
     params.stockSetModels = [...this.selectData];
    this.http.post(stockUrl.deleteStockSet,params).then((res:any)=>{
      if(res.success){
  ...
  getList1(data: any): void {
    let url = stockUrl.selectStockSet;
