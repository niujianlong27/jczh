...
export class CollectionStatisticsComponent implements OnInit {
  ...
  export() {
    ...
    this.angularHttp.post(localUrls.exportStatistics, this.searchData, {responseType: 'blob'}).subscribe(
      res => {
        ...
        a.href = objectUrl;
        a.target = '_blank';
        a.download = `收款信息统计报表${Utils.dateFormat(new Date(), 'yyyy-MM-dd HH:mm:ss')}.xlsx`;
    ...
    // this.http.post(localUrls.exportStatistics, {}).then((res: any) => {
    //   if (res.success) {
  ...
  getList(param = {}) {
    this.loading = true;
    this.http.post(localUrls.selectListByCount, param).then(
      (res: any) => {
        this.loading = false;
        if (res.success) {
