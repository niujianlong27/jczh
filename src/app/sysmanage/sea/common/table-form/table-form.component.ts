...
export class TableFormComponent implements OnInit {
  ...
  _selectedLength:any = 0;
  dataHeader:Array<any> = [];//表头
  dataHeaderRefresh:boolean = false;//判断表头是否刷新
  ...
  private colChange:boolean =false;
  @Input() closeBgTr:boolean = true;//tr换行背景色是否显示
  ...
  @Input() set selectedLength(val:any){
    // this.allChecked = true;
    this._selectedLength = val;
  }
  get selectedLength(){
  ...
  @Input() intType:boolean = false; //数字输入框是否为整数
 ...
 //列表数据
  ...
  ngOnInit() {
     ...
     this.gloColSub = this.globalSer.colChangeEmitter.subscribe((res:any)=>{
        this.colChange = true; // 标志
    ...
    //统计需要统计的字段
     for(let c of this.caculateEnameArr){
      ...
      // 2019-1-14郑鑫修改， 将每次求和都四舍五入改为将最后结果四舍五入
       this._calTotalFiled[c.field] = toDemical(this.updateData.map(item => Number(item[c.field]) || 0).reduce((acc, cur) => acc + cur, 0), c.demLength);
       c.revStr = `${c.tipInfo} ${this._calTotalFiled[c.field]} ${c.tipInfoType}`;
     }
  }
  ...
  checkAll(value: boolean): void {
   ...
   //表头获取
    getUserColumns(param:any): void{ //获取表头
      ...
      if (this.simpleCache.has(_key) && !this.colChange) {
        // 如果有缓存，则直接处理
  ...
  //处理表头信息
          ...
          this.dataHeader.forEach(
            res => {
              if (res.type === 'select') {
                ...
                res.apiParameter.valueSetCode && this.getSelectData(res.apiParameter.optionList, res.apiParameter.valueSetCode);
              }
            }
          )
        }
    ...
    columnsFilter(dataHeader:any[],type?:string){
      ...
      let checkBoxWidth = this.isCheckBox?50:0;
    ...
    currentPageDataChange(data:any): void{
      ...
      if(this._selectedLength){
        if(this._selectedLength == this._dataSet.length){
          this.allChecked = true;
        }
        const  allUnChecked = this._dataSet.filter(value => !value.disabled).every(value => !value.checked);
