...
/**
 * Title: ship-dispatch.component.ts
 * Description: 调度计划页面
 * Created: pengzitong 2019/1/24
 * Modified:
 */
@Component({
  selector: 'app-ship-dispatch',
  templateUrl: './ship-dispatch.component.html',
  styleUrls: ['./ship-dispatch.component.css']
})
export class ShipDispatchComponent implements OnInit {
  dataSet1:any = [];
  pageSize1:any = 30;
  totalPages1:any = 0;
  ...
  modalFormData: Array<any> = [
    {
      name: '调度号', eName: 'disNum', type: 'text', validateCon: '请填写调度号', require: false,placeholder:'自动生成',readOnly:true,
      validators: {
        require: false,
        pattern:false
      }
    },{
      name: '调度时间', eName: 'disDate', type: 'time', validateCon: '请填写调度时间', require: false,
      validators: {
        require: false,
        pattern:false
      }
    ...
    // {
    //   name: '调度人', eName: 'disUser', type: 'text', validateCon: '请填写调度人', require: false,
    //   validators: {
    //     require: false,
    //     pattern:false
    //   }
    // },
    {
      ...
      name: '码头合并情况', eName: 'dockMerged', type: 'text', validateCon: '请填码头合并情况', require: false,readOnly:true,
      validators: {
        require: false,
        pattern:false
      }
  ...
  getList(data:any){
    ...
    this.dataSet1 = [];
    let url = portUrl.shipDispatchList;
    let param:any = data;
   ...
   */
  btnDelete(data:any){
    let url = portUrl.shipDispatchDelete;
    let param:any = [];
  ...
  btnDeliver(data:any){
    //下发作业
    let url = portUrl.shipDispatchDeliver;
    let param:any ={boatDisDetailList: []};
    let status = false;
    this.updateData.forEach(elem => {
      if(elem.isSend == '20'){
        status = true;
      }
      param.boatDisDetailList.push({ disNum:elem.disNum,boatBatchNum: elem.boatBatchNum});
    });
    if(status){
      this.tplModal = this.nm.warning({
        nzTitle: '提示信息',
        nzContent: '存在已下发数据，请重新选择'
      });
      this.destroyTplModal();
      return;
    }
  ...
  getModalList(data:any){
    ...
    this.dataSet2 = [];
    let url = portUrl.shipDispatchModalList;
    let param:any = data;
    this.http.post(url,param).then((res:any) => {
  ...
  delete(data,index){
    ...
    if(this.buttonId == 'Update'){
      let url = portUrl.shipDispatchDelDetail;
      let param = [];
      param.push({packNo:data.packNo});
      this.http.post(url,param).then((res:any) => {
        if(res.success){
          this.nz.create('success', '提示信息', '删除明细成功！', { nzDuration: 3000 });
  ...
  handleOk(){
    ...
    let url = this.buttonId === 'Add' ? portUrl.shipDispatchAdd : portUrl.shipDispatchUpdate;
    ...
    param.boatStowageDetailList = this.dataSet2 || [];
