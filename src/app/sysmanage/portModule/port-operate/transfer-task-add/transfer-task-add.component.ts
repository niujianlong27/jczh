...
import { portUrl } from 'src/app/common/model/portUrl';
import {  NzModalService, NzNotificationService} from 'ng-zorro-antd';
import { Utils } from "../../../../common/util/utils";
@Component({
  selector: 'app-transfer-task-add',
  templateUrl: './transfer-task-add.component.html',
  styleUrls: ['./transfer-task-add.component.css']
})
export class TransferTaskAddComponent implements OnInit {
  ...
  ngOnInit() {
    this.validateForm = this.fb.group({
      moveWsNo: ['',[]],
      moveWsTime: ['', [Validators.required]],
      remark: ['', []]
    });
  ...
  btnSave():void{
    ...
    let url = portUrl.insertLoadscheduleready;
    let param:any = {loadschedulereadyDetailList: '',rowid:''};

    param = this.validateForm.getRawValue();
    console.log(param);
    param.loadschedulereadyDetailList =  this.dataSet1;
  ...
  btnClick(data: any): void {
    if(data.buttonId === 'Add'){
      ...
      sessionStorage.setItem('stockOut',JSON.stringify({stockOutDetailList:this.dataSet1}));
      this.router.navigate(['/system/stock/stockAddDetail'],{queryParams:{from:'transfer_task'}});
    }
    ...
    if(data.buttonId === 'cancel'){
      this.router.navigate(['/system/portOperate/transferTask']);
    }
  }
