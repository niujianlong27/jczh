...
export class ShipConfirmComponent implements OnInit {
  ...
  query(param: any): void {
    this.loading = true;
    this.http.post(portUrl.boatPlan, { ...param }).then(
      (res: any) => {
        if (res.success) {
  ...
  queryWork(param: any): void {
    ...
    this.http.post(portUrl.workScheduleMain, { ...param }).then(
      (res: any) => {
        if (res.success) {
  ...
  confimAudit() {
    if (!this.selectedData || this.selectedData.length === 0) {
      this.msg.error("请至少选择一条船舶计划进行确认装船操作");
      return;
    }
    this.form.reset();
  ...
  createWaybill() {
    ...
    this.http.post(portUrl.createWaybill, { ...this.form.value, boatPlans: this.selectedData }).then(
      (res: any) => {
        if (res.success) {
          this.msg.success("装船确认操作成功！");
