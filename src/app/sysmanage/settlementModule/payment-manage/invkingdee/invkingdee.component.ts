...
@Component({
  selector: 'app-invkingdee',
  templateUrl: './invkingdee.component.html',
  styleUrls: ['./invkingdee.component.css']
})
export class InvkingdeeComponent implements OnInit {
  ...
  getListSearch(data: any): void {
    ...
    const url = localUrls.selectSettleInvFinanceData;
   ...
   */
  btnExport(): void {
    if(!this.selectData || this.selectData.length < 1 || this.selectData.length > 100) {
      this.nn.warning('提示消息', '请选择少于100条的数据后进行导出！');
      return
    }
  ...
  exportRequest(): Subscription {
    const url = localUrls.exportSettleInvKingDeeData;
    const param =  this.searchData;
    param.kingDeeFinancingModels = this.selectData;
    return this.angularHttp.post(url, param, {responseType: 'blob'}).subscribe((res: any) => {
