...
@Component({
	selector: 'app-zhpark',
	templateUrl: './zhpark.component.html',
	styleUrls: [ './zhpark.component.css' ]
})
export class ZhparkComponent implements OnInit {
	tabs: Array<any> = [];
	dataSet: any = [];
   ...
   */
	getList1(data: any): void {
		...
		this.http.post(url1, { status: this.status, parkCode: 'disp.park_3' }).then((res) => {
			if (res.success) {
	...
	getallCodest(): void {
		...
		tab.itemCount = 0;
		tab.buttonId = 'All';
		tab.formId = 'form_rgDisp_zhpark0';
		this.tabs.push(tab);
		this.tabs.push({
			itemCname: '已取号',
			itemCode: 'disp_queueDetailStatus_20',
			itemCount: 0,
			formId: 'form_rgDisp_zhpark1',
			buttonId: 'TakeNumber'
		});
		this.tabs.push({
			itemCname: '确认进场',
