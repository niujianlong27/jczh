...
export class StparkComponent implements OnInit {
   ...
   */
	getList1(data: any): void {
		...
		this.http.post(url1, { status: this.status, parkCode: 'disp.park_2' }).then((res) => {
			if (res.success) {
	...
	getallCodest(): void {
		...
		tab.itemCount = 0;
		tab.buttonId = 'All';
		tab.formId = 'form_rgDisp_stpark0';
		this.tabs.push(tab);
		this.tabs.push({
			itemCname: '已取号',
			itemCode: 'disp_queueDetailStatus_20',
			itemCount: 0,
			formId: 'form_rgDisp_stpark1',
			buttonId: 'TakeNumber'
		});
		this.tabs.push({
			itemCname: '确认进场',
