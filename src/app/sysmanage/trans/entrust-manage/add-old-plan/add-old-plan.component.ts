...
@Component({
  selector: 'app-add-old-plan',
  templateUrl: './add-old-plan.component.html',
  styleUrls: ['./add-old-plan.component.css']
})
export class AddOldPlanComponent implements OnInit {
  loading: boolean;
  updatedata: any
  dataSet: any;
  totalPage: any = 0
  ...
  getList1(data: any): void {
    let url = TRANS_URLS.selectOldPlan;
    this.loading = true;
    this.dataSet = [];
  ...
  */
  btnSave(){
    if(this.updatedata.filter(x=>x.pickNo).length !==this.updatedata.length){
      this.nz.error('提示信息','请给每一个勾选的数据添加发货通知单')
      return;
    }
    let param:any={};
    let url = TRANS_URLS.saveOldPlan;
    param.planModels = this.updatedata
