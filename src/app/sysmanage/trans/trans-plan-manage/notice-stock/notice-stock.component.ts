 ...
 */
@Component({
  selector: 'app-notice-stock',
  templateUrl: './notice-stock.component.html',
  styleUrls: ['./notice-stock.component.css'],
  providers: [
    XlsxService
  ]
})
export class NoticeStockComponent implements OnInit {
  tplModal: NzModalRef;
  ...
  ngOnInit() {
    ...
    this.listSearch({page: 1, length: this.pageSize1, canSendDate: canSendDate});
  ...
  */
  ...
  getList1(data: any): void {
    let url = TRANS_PLAN_URLS.selectDeliveryNotice;
  ...
  */
  btnExport() {
    let url: any = TRANS_PLAN_URLS.exportSchedulePlan;
