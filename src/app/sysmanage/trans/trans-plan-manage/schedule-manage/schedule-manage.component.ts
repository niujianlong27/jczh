...
export class ScheduleManageComponent implements OnInit {
  ...
  logistics: any;
  chooseTime: any;
  TimeSchedulePlan:boolean = false;
  ...
  modalFormData: Array<any> = [
    {
      name: '排产计划日期', eName: 'scheduleDate', type: 'date', validateCon: '请选择排产日期', require: true,
      validators: {
        require: true,
        pattern: false
      }
  ...
  ngOnInit() {
    ...
    let scheduleDate = Utils.dateFormat(new Date(), 'yyyy-MM-dd');
    console.log(scheduleDate);
    this.listSearch({page: 1, length: this.pageSize1, scheduleDate: scheduleDate});
  ...
  getList1(data: any): void {
    let url = TRANS_PLAN_URLS.selectSchedulePlan;
    this.listLoading1 = true;
    this.tempSearchParam = data;
    console.log(data);
    console.log(this.currentTime)
    console.log(this.TimeSchedulePlan)
    if(this.TimeSchedulePlan == true){
      data.scheduleDate=this.currentTime;
    }
    //data.scheduleDate=this.currentTime;
  ...
  btnSave(): void {
    ...
    let url = TRANS_PLAN_URLS.saveSchedulePlanList;
  ...
  btnDelete(): void {
    ...
    let url = TRANS_PLAN_URLS.deleteSchedulePlanList;
    let scheduleList: any = [];
    scheduleList = this.updateData;
    this.http.post(url, scheduleList).then((res: any) => {
      if (res.success) {
  ...
  btnForecast(): void {
    ...
    let url = TRANS_PLAN_URLS.forecastSchedulePlan;
    let scheduleList: any = [];
    scheduleList = this.updateData;
    this.http.post(url, scheduleList).then((res: any) => {
      if (res.success) {
        this.nz.create('success', '提示信息', '预测完成', {nzDuration: 3000});
        this.listSearch(this.tempSearchParam);
        this.updateData = [];
      }
    });
  ...
  btnImport() {
   ...
   // this.currentTime = Utils.dateFormat(new Date(), 'yyyy-MM-dd');
    ...
    this.validateForm.get('scheduleDate').setValue(new Date());
    console.log(this.validateForm.get('scheduleDate').value)
  ...
  excelFilter(data: any) {
    let url = TRANS_PLAN_URLS.ImportSchedulePlan;
    let param: any = {scheduleList: []};
    // param.contractList = this.updateData;
    let eNameHeader: any = [];
    // this.TimeSchedulePlan=param.scheduleDate;
    // console.log(this.TimeSchedulePlan)
    console.log(param)
    ...
    if(this.currentTime instanceof Date){
      this.currentTime = Utils.dateFormat(this.currentTime, 'yyyy-MM-dd');
    }
    param.scheduleDate = this.currentTime;
    console.log(data);
    ...
    this.http.post(url, param).then((res: any) => {
      ...
      if (res.success) {
        ...
        console.log(typeof  this.validateForm.get('scheduleDate').value)
        this.listSearch({page: 1, length: this.pageSize1,scheduleDate:Utils.dateFormat(this.validateForm.get('scheduleDate').value,'yyyy-MM-dd')});
      }
    });
  ...
  */
  btnExport() {
    let url: any = TRANS_PLAN_URLS.exportSchedule;
