...
import {endOfDay, isWithinRange, startOfDay, subDays} from 'date-fns';

@Component({
  selector: 'app-outcoming-management',
  templateUrl: './outcoming-management.component.html',
  styleUrls: ['./outcoming-management.component.css']
})
export class OutcomingManagementComponent implements OnInit {
  ...
  getListSearch(data: any): void {
    ...
    const url = gcDispatchURL.getLeaveFactoryRecord;
   ...
   */
  leaveRequset() {
    const url = gcDispatchURL.LeaveFactory;
    const param: any = {};
  ...
  */
  ...
  scanCodeSuccess(){
    ...
    this.http.post(gcDispatchURL.LeaveFactory,param).then(res=>{
      if(res.success){
        this.nm.success('车辆出厂成功!');
